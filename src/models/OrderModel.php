...
class OrderModel {
    ...
    public function getMonthlyRevenue($months = 6) {
        $stmt = $this->pdo->prepare('
            SELECT DATE_FORMAT(service_date, "%Y-%m") as month, COALESCE(SUM(total_price), 0) as revenue, COUNT(id) as order_count
            FROM orders
            WHERE status IN ("paid","accepted","in_progress","completed")
              AND service_date >= DATE_SUB(CURRENT_DATE(), INTERVAL ? MONTH)
